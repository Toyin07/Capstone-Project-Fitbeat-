...
```{r}
daily_activity <- read.csv("~/R/Fitabase Data 4.12.16-5.12.16/dailyActivity_merged.csv")
daily_sleep <- read.csv("~/R/Fitabase Data 4.12.16-5.12.16/sleepDay_merged.csv")
weight <- read.csv("~/R/Fitabase Data 4.12.16-5.12.16/weightLogInfo_merged.csv")
hourly_steps <- read.csv("~/R/Fitabase Data 4.12.16-5.12.16/hourlySteps_merged.csv")
intensities <- read.csv("~/R/Fitabase Data 4.12.16-5.12.16/hourlyIntensities_merged.csv")
...
#Remove duplicates.
sleep_day <- daily_sleep[!duplicated(daily_sleep), ]
sum(duplicated(daily_sleep))

#Add a new column for the weekdays
daily_activity <- daily_activity %>% mutate( Weekday = weekdays(as.Date(ActivityDate, "%m/%d/%Y")))

merged1 <- merge(daily_activity,daily_sleep,by = c("Id"), all=TRUE)
...
Step 6: Analysis on active minutes, calorie, total steps. 
```{r}
#Active users
...
```
Step 7: Regression analysis and R value, leverage points 
```{r}
